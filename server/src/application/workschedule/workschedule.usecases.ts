...
export class WorkScheduleUseCases {
  ...
  createNewSchedule = async (newSchedule: WorkScheduleValueDTO): Promise<WorkSchedule> => {
    ...
    const existingAssignment = await this.workScheduleRepo.findByDocumentAndDate(newSchedule.employee, newSchedule.assignedDate)
  ...
  findWorkScheduleByDocumentAndDate = async (document: string, newDate: string) => {
    const schedule = await this.workScheduleRepo.findByDocumentAndDate(document, newDate);
  ...
  findWordkSchedulesByStoreId = async (storeId: string): Promise<WorkSchedule[] | null> => {
    const workSchedules = await this.workScheduleRepo.findByStoreId(storeId);
